...
if (User::getAuth()->isLoggedIn()) {
    if (User::canEditAsEditor()) {
        ...
        $router->post('/documentation/(\w+)/(\d+)(/\w+)?(/[a-z0-9_-]+)?', function ($action, $id, $template_route = null, $template_id = null) use ($auth) {
            ...
            $redirect = Path::get('BASE_PATH') . (isset($template_id) && $action != 'delete' ? htmlentities($template_route) . DIRECTORY_SEPARATOR . htmlentities($template_id) : "");
            ...
            switch ($action) {
                ...
                case 'insert':
                    ...
                    $documentation->insert(['item_template_id' => $id, 'title' => 'Title', 'subtitle' => 'Subtitle', 'subject' => DatabaseManager::connection()->selectValue('SELECT `id` FROM `tag` LIMIT 0, 1', [])]);
                    $documentation_id = DatabaseManager::connection()->getLastInsertId();
                    $item->insert(['template' => $id, 'parent' => $parent_id, 'slug' => $slug]);
                    $documentation->where('id', $documentation_id)->update(['item_id' => DatabaseManager::connection()->getLastInsertId()]);
                ...
                case 'update':
                    ...
                    $redirect = (isset($template_id) && $action != 'delete' ? htmlentities($template_route) . DIRECTORY_SEPARATOR . htmlentities($slug) : "");
...
$router->get('/documentation/([a-z0-9_-]+)', function ($slug) {
    ...
    $documentation = (new \Documentation\Item)->where('id', $item->table_id)->fetchOne();
