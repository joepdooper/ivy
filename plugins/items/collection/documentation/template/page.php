    ...
    <div class="row">
                        ...
                        <li>
                            ...
                            <ul>
                                <?php foreach ((new \Documentation\Item)->join('items', 'item_id', '=', 'id')->where('subject', $tag->id)->get()->all() as $link): ?>
                                    <?php if ($link->published || User::getAuth()->isLoggedIn()): ?>
                                        <li class="<?php if ($link->id === $item->id): ?>active<?php endif; ?>">
                                            <a href="<?= Path::get('BASE_PATH') . 'documentation/' . $link->slug; ?>">
        ...
        <div class="col-12 col-9-md">
            <article>
                ...
                <?php if (User::getAuth()->isLoggedIn() && $item->author): ?>
                <form action="<?= Path::get('BASE_PATH') . 'documentation/update/' . $item->id . Template::$url; ?>"
                    ...
                    <div class="p-1">
                        ...
                        <?php if (isset($documentation->subject)): ?>
                            <!-- Subject -->
                            <div class="inner">
                                <?php $tag = (new \Tag\Item)->where('id', $documentation->subject)->fetchOne(); ?>
